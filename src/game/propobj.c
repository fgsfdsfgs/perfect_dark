...
struct map weaponMap[] = {
	...
	{ 998,                     "DrugSpy"},
	{ 997,                     "BombSpy"},
	{ WEAPON_XRAYSCANNER,      "X-Ray Scanner"},
	{ WEAPON_IRSCANNER,        "IR Scanner"},
	{ WEAPON_CLOAKINGDEVICE,   "Cloaking Device"},
	{ WEAPON_HORIZONSCANNER,   "Horizon Scanner"},
	{ WEAPON_ECMMINE,          "ECM Mine"},
	{ WEAPON_DATAUPLINK,       "Data Uplink"},
	{ WEAPON_RTRACKER,         "R-Tracker"},
	{ WEAPON_PRESIDENTSCANNER, "President Scanner"},
	{ WEAPON_DOORDECODER,      "Door Decoder"},
	{ WEAPON_AUTOSURGEON,      "AutoSurgeon"},
	{ WEAPON_EXPLOSIVES,       "Explosives"},
	{ WEAPON_SKEDARBOMB,       "Skedar Bomb"},
	{ WEAPON_COMMSRIDER,       "Comms Rider"},
	{ WEAPON_TRACERBUG,        "Tracer Bug"},
	{ WEAPON_TARGETAMPLIFIER,  "Target Amplifier"},
	{ WEAPON_DISGUISE40,       "Disguise"},
	{ WEAPON_DISGUISE41,       "Disguise"},
	{ WEAPON_FLIGHTPLANS,      "Flight Plans"},
	{ WEAPON_RESEARCHTAPE,     "Research Tape"},
	{ WEAPON_BACKUPDISK,       "Backup Disk"},
	{ WEAPON_KEYCARD45,        "Key Card"},
	{ WEAPON_KEYCARD46,        "Key Card"},
	{ WEAPON_KEYCARD47,        "Key Card"},
	{ WEAPON_KEYCARD48,        "Key Card"},
	{ WEAPON_KEYCARD49,        "Key Card"},
	{ WEAPON_KEYCARD4A,        "Key Card"},
	{ WEAPON_KEYCARD4B,        "Key Card"},
	{ WEAPON_KEYCARD4C,        "Key Card"},
	{ WEAPON_SUITCASE,         "Suitcase"},
	{ WEAPON_BRIEFCASE,        "Briefcase"},
	{ WEAPON_SHIELDTECHITEM,   "Shield"},
	{ WEAPON_NECKLACE,         "Necklace"},
	{ WEAPON_SUICIDEPILL,      "Suicide Pill"},
	{ WEAPON_ROCKET,           "Rocket"},
	{ WEAPON_HOMINGROCKET,     "Homing Rocket"},
	{ WEAPON_GRENADEROUND,     "Grenade Round"},
	{ WEAPON_BOLT,             "Bolt"},
	{ WEAPON_BRIEFCASE2,       "Briefcase"},
	{ WEAPON_SKROCKET,         "Rocket"},
};
	...
	*dst = '\0';
	...
	if (info != NULL) {
		...
		if (nametextid != 0) {
			...
			if (determinertextid) {
				sprintf(buffer, "%s%s", _(determinertextid), _(nametextid));
			} else {
				sprintf(buffer, "%s", _(nametextid));
			}
	...
	if (doorIsUnlocked(playerprop, doorprop)) {
		...
		if ((door->base.flags2 & OBJFLAG2_SKIPDOORLOCKEDMSG) == 0) {
			...
			if (override && override->pickuptext) {
				hudmsgCreateWithFlags(_(override->pickuptext), HUDMSGTYPE_DEFAULT, HUDMSGFLAG_ONLYIFALIVE);
			} else if (intraining) {
				hudmsgCreateWithFlags(_("Cannot exit while training is in progress.\n"), HUDMSGTYPE_DEFAULT, HUDMSGFLAG_ONLYIFALIVE); // "Cannot exit while training is in progress."
			} else {
				hudmsgCreateWithFlags(_("This door is locked.\n"), HUDMSGTYPE_DEFAULT, HUDMSGFLAG_ONLYIFALIVE); // "This door is locked."
			}
		}
